...
const service = EmailService.enhanceEndpoints({
  ...
  endpoints: (build) => ({
    getTemplatesList: build.query<any, any>({
      query: () => ({
        method: "GET",
        url: `template/?folder=${import.meta.env.VITE_MAIL_RESCHEDULE_TEMPLATE_FOLDER_ID}`
      }),
    ...
    getTemplate: build.query<any, any>({
      query: () => ({
        method: "GET",
        url: `template/6071/?folder=${import.meta.env.VITE_MAIL_RESCHEDULE_TEMPLATE_FOLDER_ID}`
      }),
      providesTags: ["emailApi"],
    }),
