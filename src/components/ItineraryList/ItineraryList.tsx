...
const ItineraryList: React.FC<ItineraryListProps> = ({
  ...
  const [enabledDatePickers, setEnabledDatePickers] = useState<number[]>([0]);
  const [, SsetModifyDates] = sessionStorageAccessor<any>("modifyDates");
  const { isSmallScreen, isMediumScreen } = useResize();

  // useEffect(() => {
  //   var checkModify = [];
  //   if (action?.content?.toLowerCase() === "modified") {
  //     var nonConfirmedFlights = flight_details?.map((pnr: any) =>
  ...
  //       checkModify = flight_details?.filter(
  ...
  useEffect(() => {
    // Only proceed if we have flight details and action content matches
    if (!flight_details || !action?.content) return;

    const checkModify = action.content.toLowerCase() === "modified" 
      ? flight_details.map((pnr) => 
          pnr.flightDetails.filter((flight) => flight.statusCode !== "HK")
        ).filter((item) => item.length)
      : action.content.toLowerCase() === "custom" 
        ? flight_details.filter((flight:any) => flight?.itinerary_status === "modify")
        : [];

    // Only update if we have actual changes
    if (selectedDates.length !== checkModify.length) {
      sendDataToParent("", "", true, false);
    } else if (selectedDates.length === checkModify.length) {
      sendDataToParent("", "", true, true);
    }
    
    // Only dispatch if selectedDates actually changed
    dispatch(updateModifyDates(selectedDates));
    SsetModifyDates(selectedDates);
  }, [selectedDates, action, flight_details]);
  ...
  const handleDateChange = (date: Dayjs | null, flightId: number) => {
    ...
    setSelectedDates((prevDates) => {
      if (formattedDate === "") {
        ...
        return prevDates?.filter((entry) => entry.flightId !== flightId);
      }
    
      // Create the new entry
      const newEntry = { flightId, date: formattedDate, dateData: date };
      // Check if the entry already exists
      const existingIndex = prevDates.findIndex((entry) => entry.flightId === flightId);
    
      // If it exists, update; otherwise, add a new entry
      return existingIndex !== -1
        ? prevDates?.map((entry, index) => (index === existingIndex ? newEntry : entry))
        : [...prevDates, newEntry];
    });

    const resFlightCount = flight_details?.map((data) => 
      !data?.flightDetails.some((flight) => flight.statusCode !== "HK")
    ).filter((data) => !data);

    if (resFlightCount?.length === enabledDatePickers.length) {
      sendDataToParent("", "", true, true);
    } else if (!enabledDatePickers.includes(flightId + 1)) {
  ...
  if (!flight_details?.length) {
    return <ItineraryListSkeleton data-testid="ItineraryList" />;
  }

  return (
    <div data-testid="ItineraryList" className="cls-itineraryListMain">
      <div className="hide-res-only">
        <Row justify="space-between" className="mt-15 mb-20 cls-heading">
      ...
      <div className={`cls-pnrlistv1-2 cls-action ${action?.class}`}>
        ...
        {flight_details?.map((data: any, mainIndex: number) => {
          return (
            <div key={`data-${mainIndex}`} className="cls-accept" data-testId="flightCard">
              <Text
                className={`d-iblock p-clr cls-trip ${isSmallScreen || isMediumScreen ? "pt-0" : "pt-15"}`}
                key={`trip-${mainIndex}`}
              >
                {" "}
                Trip {data.trip}
              ...
              {data.flightDetails?.map((item: any, index: number) => (
                <div
                  ...
                  className={`${data.flightDetails.length === index + 1 && flight_details.length !== mainIndex + 1 ? "bordered-row" : ""} ${isSmallScreen || isMediumScreen ? "pt-15 pb-10" : "pt-10 pb-15"}`}
                >
                  <Row justify="space-between" className="relative">
                    <Col xs={6} sm={6} md={6} lg={4} xl={4} className="fs-15 f-sbold hide-res-only">
                      <Text className="mr-15">{item?.originAirportCode}</Text>{" "}
                      <FdArrowRightIcon />
                    ...
                    <Col xs={24} md={24} lg={3} xl={4} className={`f-reg cls-depDate ${isSmallScreen || isMediumScreen ? "fs-13 h-24 py-25" : "fs-15"}`}>
                  ...
                  data.flightDetails?.some((item:any) => item?.statusCode !== "HK") ? (
                    <Row key={`modified-${mainIndex}-${index}`} className="cls-dateRow">
                      <Col
                        className={`${isSmallScreen || isMediumScreen ? "mt-15 fs-13" : "mt-25"}`}
                        style={{
                  ...
                  data.flightDetails?.some((item:any) => item?.statusCode !== "HK") ? (
                    <>
                      <Row
                        className="mt-25 mb-5 mr-15 cls-customModifyRow"
                        align="middle"
                        wrap
                      ...
                      >
                        ...
                        >
                          {data?.itinerary_status?.toLowerCase() === "modify" ? (
                            <>
                              <Text 
                                className={`${isSmallScreen || isMediumScreen ? "fs-13 f-reg w-100 d-block mt-5" : ""} cls-grey`}
                              >
                                {t("select_date_msg")}
                              </Text>
                              <DatePicker
                                className={`${isSmallScreen ? "my-10" : "ml-15"}`}
                                size="middle"
                                inputReadOnly={true}
                                allowClear={false}
                                showToday={false}
                        ...
                        <Col xs={24} md={24} lg={24} xl={8} className="text-right">
                          <Radio.Group
