...
const AddOnPopup = (props: AddOnPopupProps) => {
  ...
  const [, SsetSsrPrice] = sessionStorageAccessor<any>("ssrPrice");
  ...
  const content = (
    <Text className="cls-price-info-popup">
      <Flex justify="space-between" className="pt-10">
        <Text>
      ...
      <Flex justify="space-between" className="py-10 cls-bdr-btm">
        <Text>
      ...
      <Flex justify="space-between" className="pt-10 pb-5 cls-bdr-btm">
        <Text>
  ...
  return props.currentTab === "modify" ? (
        ...
        >
          <Col xs={4} lg={16} xl={19} className="cls-selected-data d-flex" style={{ overflowX: "auto" }}>
            {props?.selectedFlights?.map((flight: any, itemIndex: number) =>
              <React.Fragment key={"modifyPopup" + itemIndex}>
                <Col className="cls-float-body text-center mr-15 d-flex">
                  <Text>
                  ...
                  <Text className="d-flex">
                    <Text className="fs-16 f-bold mr-10">
                      {flight?.originAirportCode}
                    </Text>
                    <Text className="cls-icon-center cls-line mx-5">
                      ------<Text className="Infi-47_FlightStart"></Text>
                    </Text>
                    <Text className="fs-16 f-bold ml-10">
                  ...
                  {props.selectedFlights.length !== itemIndex + 1 && (
                    <Divider
                      ...
                      style={{
                        height: "40px",
                        borderColor: "var(--t-text-grey-light)"
                      }}
                    />
                  )}
                </Col>
          ...
          <Col xs={20} lg={8} xl={5} className="cls-float-popup-price">
            <Button
              type="default"
              size="large"
              className={`ml-15 fs-18 f-med ${props.selectedFlights.length === original_flight_data.length
                  ? ""
                  : "disabled no-events"
                }`}
              onClick={() => {
            ...
            {/* {enableNext && props.selectedFlights.length !== original_flight_data.length ?
              <Button
                type="primary"
                size="large"
                className={`px-25 ml-15 fs-18 f-med`}
                onClick={() => {
                  setEnableNext(false);
                  if (props.tripSelect) {
              ...
              <Button
                type="primary"
                size="large"
                className={`px-25 ml-15 fs-18 f-med ${isSmallScreen ? "cls-primary-btn" : ""} ${props.selectedFlights.length === original_flight_data.length ? "" : "disabled no-events"}`}
    ...
    <Row data-testid="AddOnPopup" className="cls-float-popup" align="middle" justify="space-between">
      <Col>
        <Row>
          {/* <Col className="pr-30 cls-dashed">
      ...
      <Col>
        <Row>
          <Col className="pr-20 cls-dashed">
            ...
            <Text>
              ...
              <Popover content={content}>
                <Text className="Infi-15_Info cls-infoIcon"></Text>
              </Popover>
            </Text>
          </Col>
          <Col className="ml-20 cls-float-popup-price">
            <Button
