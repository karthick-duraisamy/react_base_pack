...
const AdhocPnrList = ({ cardsData, clusterData, handlePNRData, clusterIndex, handleFilterData, filterCluster="Y" }: any) => {
  const [filterPNR, setFilterPNR] = useState(filterCluster);
  ...
  return (
    <Row className="cls-AdhocPNRList" data-testid="AdhocPNRList">
      ...
      <Col span={24} className="cls-cluster-div">
        <Text className="fs-17 f-med s-clr-bg d-iblock pr-30 pb-5">Filter by cluster type</Text>
        <Radio.Group
          options={[
            { "label": "Yes", "value": "Y" },
            { "label": "No", "value": "N" }
          ]}
          defaultValue={filterPNR}
        ...
        {filterPNR === "Y" &&
          <Row>
            <Radio.Group
              ...
              className="d-flex flex-wrap g-10 w-100 pt-10"
              onChange={(e) => {
            ...
            >
              {clusterData?.map((data: any, index: number) => (
                <Col  xs={24} sm={10} md={10} lg={7} xl={6}  className="mr-20">
                  <Radio.Button value={"itinerary_" + index}>
                    <Card>
                      <Text className="d-block fs-15 f-reg"> {data.segment} <Text className="fs-13 f-reg cls-grey-lite pl-5"> {data.stops}{" "} stops</Text></Text>
                      <Text className="d-block fs-15 f-reg">
                        Pax {" "} {data.paxCount}
                        <Text className="cls-grey-lite fs-16"> | </Text>
                        {data?.info?.map((pax:any, paxIndex:number) => (
                          <Text 
                            className="fs-15 f-med pr-5" 
                            style={
                              { 
                                color: 
