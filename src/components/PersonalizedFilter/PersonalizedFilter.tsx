...
import { DatePickerRe, FdFilter } from "../Icons/Icons";
...
type DynamicOptionsType = {
  key: string;
  type: "default" | "split" | "date" | "datepicker";
  ...
  return (
    <Row className="cls-personalized-filter" gutter={isSmallScreen ? 0 : 40} justify="end">
      {!!Object.keys(appliedFilters)?.length && (
        <Col>
          <Flex align="center" justify="center" className="cls-applied-filters">
            { tableDataPreparationHandler &&
              <Text type="secondary" className="fs-14 mr-15">
                Applied Filters :
              </Text>
            }
            <Space>
              {appliedFilterKeys
                ...
                ?.map((f: string, index: number) => {
                  return (
                    <Card className="cls-applied-filter-card" key={`${index}${visibleColumns}`}>
                      <Text>{(appliedFilters as any)?.[f]}</Text>
                      <Button type="link" className="px-0 py-0">
                        <Text
                          className="Infi-82_CloseMark fs-10 cls-close-mark ml-10"
      ...
      <Col className="cls-filter-dropdown">
            ...
            <Card className="cls-dropdown" bordered={false} style={{minWidth: 247}}>
              ...
              >
                <Flex
                  ...
                  className="cls-filter-header py-10"
                >
                  <Text>
                    <Text title="Go back" className="Infi-10_ArrowRight cls-goBack" onClick={()=>toggleOpen()}></Text>
                ...
                <Flex vertical gap={8} className="cls-filters pt-10 py-15">
                { tableDataPreparationHandler &&
                  <Form.Item
                  ...
                  <Button
                    htmlType="submit"
                    type="default"
                    className="mt-20 w-100 cls-submit"
                  >
                    Apply
                  </Button>
                  { tableDataPreparationHandler &&
          ...
          <Card className="cls-dropdown" bordered={false} style={{minWidth: 247}}>
              ...
              >
                ...
                { tableDataPreparationHandler &&
                  ...
                  {filters.slice(0, 4).map((filter: CustomFilterType) => {
                    const options: OptionType[] = generateOptions(
                      tableData,
                      filter.option
                    );
        ...
        <Tooltip title={t("add_filter")}>
          <Button type="primary" className="d-block px-5 mr-10" onClick={() => toggleOpen()}>
            <FdFilter />
          </Button>
          {/* <Text className="Infi-11_FilterMenu fs-22"></Text> */}
        </Tooltip>
        {contextHolder}
      </Col>
      {/* Save filter modal */}
