...
const PrePlannedDisruptedPNRList = (props: any) => {
  ...
  const { currentPath, redirect, getEncryptedPath, isCurrentPathEqual } = useRedirect();
  // const [, SsetFormData] = sessionStorageAccessor<any>("formData");
  const { queuePnrs, reloadPnrList } = useAppSelector(
    (state: any) => state.PnrListReducer
  );
  // const [filterTab, setFilterTab] = useState<string>("all");
  const [tableData, setTableData] = useState<any>([]);
  const [_tabOptions, setTabOptions] = useState<any>();
  ...
  //   sessionStorageAccessor<any>("adhocFlightPNRs");
  const [SgetPNRQueueOption, SsetPNRQueueOption] =
    sessionStorageAccessor<any>("PNRQueueOption");
  const pnrQueue = useGetPNRData("pnrQueue");
  const [, SsetFinalViewPnrData] = sessionStorageAccessor<any>("finalViewPNRData");
  const [, SsetPrevPath] = sessionStorageAccessor("prevPath");
  ...
  const tableDataChange = () => {
    pnrQueue?.forEach((pnrList: any) => {
      if (SgetPNRQueueOption() === pnrList.purposeName) {
  ...
  useEffect(() => {
    SsetPNRQueueOption(SgetPNRQueueOption() ? SgetPNRQueueOption() : "Disruption data");
    setTableData([]);
  }, []);
        ...
        label: (
          <>
            <Text
              ...
              style={{
                color:
                  status === "Accepted" ? "var(--t-green-light)" : "",
              }}
  ...
  const [_pageTableData, setPageTableData] = useState<any[]>([]);
  ...
  const getURL = (pnr: any) => {
    const redirectUrl = `${import.meta.env.VITE_REDIRECT_URL}${getEncryptedPath("viewPnrInfo").slice(1)}?${encryptData(`${pnr.PNR}.${pnr.lastName}`)}`;
  ...
  const prepareTableData = (pnrData: any) => {
    return {
            ...
            >
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please enter a valid email ID or 10-digit phone number!",
                  },
                ]}
              >
                <Input
                  ...
                  onKeyDown={(event) => {
                    const isValid =
                      UseInputValidation({ type: "email", event: event }) ||
                      UseInputValidation({ type: "mobileNumber", event: event });
      ...
      alertStatus: (
        ...
        >
          <Tooltip
            className={`pointer ${pnrData?.emailId === "" ? "disabled no-events" : ""}`}
      ...
      action: (
        ...
        >
          <Text
            className="fs-23 d-block w-100 text-center cursor pointer"
            onClick={() => {
              if (
                isCurrentPathEqual("prePlanned") ||
  ...
  const prepareEmailFlightData = (pnrData: any = {}) => {
    ...
    const mapFlightDetails = (originalFlightDetails = [], _pnr = "") => {
    ...
    return {
      ...
      product_link: `${import.meta.env.VITE_REDIRECT_URL}${getEncryptedPath("viewPnrInfo").slice(1)}?${encryptData(`${pnr}.${pnrData.lastName}`)}`,
    };
  };
    ...
    ...(isCurrentPathEqual("prePlanned")
      ...
      : [
        ...
        },
        {
          ...
          render: (remainingPax: any) => {
            return <Text className="f-reg fs-13">
              {remainingPax}
              <Text className="pl-5 fs-11 f-reg">Pax remaining</Text>
            </Text>;
          },
        },
        {
        ...
        },
        {
          ...
          render: (_: any, { reassignedRoute, reassignedFlightNo, reassignedDepartureDate }: any) => {
            return (
              <>
        ...
        return (
          <>
            ...
            <Text
              ...
              style={{
                color: status === "Accepted" ? "var(--t-green-light)" : "",
              }}
  ...
  const [filterTab, _setFilterTab] = useState<string>("all");
        ...
        <Row className="cls-disrupted-pnr-list">
          <Col span={24} className={"mb-20"}>
            <Row className="rg-10" justify="space-between" align="middle" gutter={10}>
              ...
              {!isCurrentPathEqual("prePlanned") &&
                <Col lg={5} xs={24}>
                  <Text className="fs-20 pb-10 f-med d-block p-clr">PNR details list</Text>
                </Col>
              }
              {!!customFilterProps.length && (
                <Col lg={!isCurrentPathEqual("prePlanned") ? 15 : 18} xs={24} className={isResponsive ? "pb-10" : ""}>
                  <PersonalizedFilter
                    tableData={queuePnrs}
                    filters={customFilterProps}
                    visibleColumns={visibleColumns}
              ...
              <Col lg={4} xs={24} className={isResponsive ? "pb-10" : ""}>
              ...
              {isPNRSelected && (
                <Col span={2}>
                  <Select
                    ...
                    suffixIcon={
                      <Text className="Infi-04_DropdownArrow fs-10 cls-dropdown-arrow"></Text>
                    }
                  />
                </Col>
              )}
          ...
          >
            ...
            {/* <Text className="mt-5 mb-10 d-iblock">
