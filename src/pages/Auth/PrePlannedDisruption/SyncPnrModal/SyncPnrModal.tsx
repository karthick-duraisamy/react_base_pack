...
const SyncPnrModal = ({
  ...
  const [SgetPNRQueueOption, SsetPNRQueueOption] = sessionStorageAccessor<any>("PNRQueueOption");
  ...
  useEffect(() => {
    ...
    if (queueData) {
      ...
      selectedQueue = queueData.filter((queue:any) => 
          page === "prePlanned" 
          ? queue.purposeName === SgetPNRQueueOption()
          : queue.purposeName === SgetFlightQueueOption()
        );
      selectQueueOrAPIHandler(
        JSON.stringify(
          selectedQueue[0]), page === "prePlanned" 
            ? SgetPNRQueueOption() 
            : SgetFlightQueueOption(),
          "queue"
      );
    }
  ...
  const syncPNRHandler = async (_value: any) => {    
    setLoading(true);
    try {
  ...
  const [sampleXlsxData, _setSampleXLSXData] = useState<any[]>(
    page === "prePlanned"
      ? [
          [
            "PNR",
  ...
  return (
    <>
      <Modal
        ...
        footer={false}
        closeIcon={[
          <Text className="Infi-09_CloseIcon cls-modal-close-icon"></Text>,
        ]}
        cancelText="Cancel"
        centered={true}
      >
        ...
        >
          <Tabs
            className={`Templates ${loading ? "disabled no-events" : ""}`}
          ...
          ></Tabs>
          <Row align="middle" justify="space-between" className="mt-25">
            <Col xs={24} sm={12} md={8} lg={8} xl={8}>
              {activeTab === "file" && (
                <Button
                  type="default"
                  onClick={handleDownload}
                  className="cls-download-btn w-100 px-10 py-20 my-10"
                >
                  <XlsxDownload />{" "}
