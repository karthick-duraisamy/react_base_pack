...
const AdhocDisruptionList: React.FC = () => {
  ...
  const [, SsetFlightDetail] = sessionStorageAccessor<any>("flightDetail");
  ...
  const [, SsetAdhocFlightPNRs] = sessionStorageAccessor("adhocFlightPNRs");
  const [SgetFlightQueueOption, SsetFlightQueueOption] = sessionStorageAccessor<any>("FlightQueueOption");
  const flightQueue = useGetPNRData("flightQueue");

  useEffect(() => {
    SsetFlightQueueOption(
      SgetFlightQueueOption() ? SgetFlightQueueOption() : "rescheduled"
    );
  }, []);

  useEffect(() => {
    flightQueue?.forEach((flightList: any) => {
      if (SgetFlightQueueOption() === flightList.purposeName) {
  ...
  const [filterTab] = useState<string>("all");
      ...
      <Row className="cls-adhocList" data-testid="AdhocDisruptionList">
        <Col lg={24} xs={24}>
            ...
            <Col lg={24} xs={24}>
              ...
              >
                {cardsData?.map((card: any, index: number) => (
                  <Text className={`cls-card-span d-block ${index !== 0 ? "pl-15" : ""}`}>
                      ...
                      <Row style={{ padding: 0 }}>
                        ...
                        <Col lg={18} xs={17}>
                          <Text className="cls-disrupted-total d-flex fs-14 f-reg">
                            {" "}
                            {card?.description}{" "}
                            <Text className="f-med pl-5 fs-14">
                              {card?.descriptionCount}
                            </Text>
                          </Text>
                          <Text className="cls-pending fs-24 pt-10 pr-5 d-iblock p-clr">
                            {card?.pendingCount}
                          </Text>
          ...
          {!adhocList?.length ? (
            ...
            <Card bordered={false}>
              <Row>
                <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mt-5 mb-10">
                  <Row className="rg-10" align="middle">
                    ...
                    <Col xs={20} sm={14} md={23} lg={8} xl={6} className="cls-grey fs-13 f-reg">
                      ...
                      <Select
                        ...
                        style={{ width: 220 }}
                        className="pl-5 cls-sort-select"
                        allowClear
                      >
                      ...
                      <Col xs={4} sm={10} md={1} lg={8} xl={10} className={`${isResponsive ? "pb-10" : "pl-10"}`}>
                        {!!customFilterProps.length && (
                          <PersonalizedFilter
                            tableData={adhocList}
                            filters={customFilterProps}
                    ...
                    <Col xs={24} sm={24} md={24} lg={5} xl={5} className={`${isResponsive ? "pb-10" : "pr-15"}`}>
                    ...
                    <Col xs={24} sm={24} md={24} lg={3} xl={3}>
                      <Button
                        className="cls-primary-btn px-5 h-32 w-100"
                        onClick={() => toggleModal()}
                      >
